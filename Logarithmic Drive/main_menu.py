...
from logarithmic_drive import DXF_UNITS_LABEL
...
def discover_demo_entries(repo_dir: Path = REPO_DIR) -> list[DemoEntry]:
    return [
        DemoEntry(
            title="Novel Ball-Bearing Logarithmic Drive",
            description=(
                "Uses the Promakina logarithmic-drive profile equations: a red ellipse drives a blue hypocycloid groove through "
                "green ball elements retained in gray holes."
            ),
            path=repo_dir / "demo_logarithmic_1.py",
        ),
    ]


class LogarithmicHubPage(ttk.Frame):
    def __init__(self, master, on_back=None, title_callback=None, launch_python: str | None = None) -> None:
        ...
        self.detail_description_var = tk.StringVar(
            value="Logarithmic drive demos launch in their own windows and export DXFs from the starting pose."
        )

        self._build_ui()
        self._populate_entries()
        self.title_callback("Logarithmic Drive")

    def _build_ui(self) -> None:
        self.columnconfigure(0, weight=1)
        self.rowconfigure(2, weight=1)

        ttk.Label(self, text="Logarithmic Drive", font=("SF Pro Display", 20, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Label(
            self,
            text="Launch logarithmic reducer demos in the same outline-first house style. Use Back to return to the reducer families.",
        ...
        ttk.Label(detail_frame, textvariable=self.detail_path_var, wraplength=220, justify="left").grid(row=2, column=0, sticky="w")
    ...
    def _handle_selection(self, _event=None) -> None:
        ...
        self.title_callback(f"{entry.title} - Logarithmic Drive")
...
class DemoMenuApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("Logarithmic Drive")
        self.root.geometry("760x520")
        self.root.minsize(700, 460)
        self.root.configure(bg="#f4f1e8")

        self.page = LogarithmicHubPage(root)
        self.page.pack(fill="both", expand=True)


def build_embedded_page(master, on_back=None, title_callback=None, launch_python: str | None = None):
    return LogarithmicHubPage(master, on_back=on_back, title_callback=title_callback, launch_python=launch_python)
