...
#plt.grid()
t = np.linspace(0, 2*np.pi, POINT_SAMPLE_COUNT)
delta = 1
mode_fig = 0
pin_fig = 0
cycloid_fig = 1  
...
animation_state = {"phase": 0.0}
rax = plt.axes([0.025, 0.64, 0.15, 0.14], facecolor=axcolor)
radio = RadioButtons(rax, ('Motion_1', 'Motion_2'), active=0)
rax_1 = plt.axes([0.025, 0.48, 0.15, 0.14], facecolor=axcolor)
radio_1 = RadioButtons(rax_1, ('Pin', 'NonePin'), active=0)
rax_2 = plt.axes([0.025, 0.32, 0.15, 0.15], facecolor=axcolor)
radio_2 = RadioButtons(rax_2, ('Single', 'Dual', 'Triple'), active=0)

rax_3 = plt.axes([0.025, 0.80, 0.15, 0.14], facecolor=axcolor)
...
def animate(_frame):
    ...
    if sfm == 0:
        phi = animation_state["phase"]
    if sfm != 0:    
        phi = animation_state["phase"] + (2*np.pi / sfm)
        animation_state["phase"] = np.mod(phi, 2 * np.pi * max(sN - 1, 1))
    ...
    if cycloid_fig == 1:          
        ...
        edotB.set_visible(0)
        edotC.set_visible(0)
ani = animation.FuncAnimation(fig, animate, frames=itertools.count(), interval=INTERVAL_MS, cache_frame_data=False)
dpi=100
plt.show()
